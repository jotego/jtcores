...
blocks for the two CPU register banks. The MMR outputs are internal wires so the
module can grow into a full K054156/K054157 implementation.
...
- `hdl/jt05415x.v`: wrapper for the chip pair register maps.
...
Next implementation work should consume the internal MMR wires in
`hdl/jt05415x.v` and progressively replace the documentation-only behavior
with synthesizable FPGA logic for VRAM access, tile addressing, scroll
generation, K054157 color/readout paths, and ROM readback.
