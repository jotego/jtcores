...
reg              saw_done = 1'b0;
reg [12:0]       active_row = 0;
reg [VW:0]       write_base = 0;
reg [AW-1:0]     check_addr;
reg [15:0]       sdram_mem [0:(1<<AW)-1];
...
always @(posedge clk) begin
    if( !rst && sdram_cmd == CMD_ACTIVE ) active_row <= SDRAM_A;
    if( !rst && check_writes && fb_done ) saw_done = 1'b1;
    if( !rst && check_writes && sdram_cmd == CMD_WRITE ) begin
        sdram_x = SDRAM_A[HW-1:0];
        if( fb_keep && active_row[VW] !== 1'b0 ) begin
            $display("FAIL: SDR keep write used row bank %0d, expected fixed bank 0", active_row[VW]);
            fail();
        end
        if( { SDRAM_DQML, SDRAM_DQMH } == 2'b11 ) begin
            masked_count = masked_count + 1;
        end else if( { SDRAM_DQML, SDRAM_DQMH } == 2'b00 ) begin
            if( write_count == 0 ) write_base = active_row[VW:0];
            sdram_mem[{ active_row[VW:0], sdram_x[HW-1:0] }] = SDRAM_DQ;
            write_count = write_count + 1;
        end else begin
            $display("FAIL: partial SDR byte mask at x=%0d DQML=%b DQMH=%b",
                sdram_x, SDRAM_DQML, SDRAM_DQMH);
            fail();
        end
    end
end
...
begin
    ...
    if( !saw_done ) begin
        $display("FAIL: timed out waiting for SDR line write completion");
        errors = errors + 1;
    end
    if( write_count != expect_writes ) begin
        $display("FAIL: observed %0d SDR writes, expected %0d", write_count, expect_writes);
        errors = errors + 1;
    end
    if( masked_count != expect_masked ) begin
        $display("FAIL: observed %0d SDR masked writes, expected %0d", masked_count, expect_masked);
        errors = errors + 1;
    end
end
endtask
...
begin
    for(i=0; i<LINE_W; i=i+1) begin
        ...
        if( sdram_mem[check_addr] !== expected ) begin
            $display("FAIL: SDR storage addr=%0d got %04x expected %04x",
                i, sdram_mem[check_addr], expected);
            errors = errors + 1;
        end
    end
end
...
initial begin
    for(i=0; i<(1<<AW); i=i+1) sdram_mem[i] = 16'hxxxx;
    ...
    pulse_vs();
    if( frame !== 1'b1 ) begin
        $display("FAIL: frame did not toggle before SDR keep-bank check");
        errors = errors + 1;
    end
