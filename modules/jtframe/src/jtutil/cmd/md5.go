...
func init() {
	md5Cmd := &cobra.Command{
		...
		Long:  man_blurb("jtutil-md5", "Update the MD5 sum values recorded for MRA files."),
		Run:   run_md5_command,
	}
...
func run_md5_command(cmd *cobra.Command, args []string) {
	...
	newfile, e := list_md5(mrapath)
	must(e, fmt.Errorf("while listing md5"))
	reference, _ := cmd.Flags().GetString("compare")
	if reference != "" {
		diff, e := compare(newfile, reference)
		must(e)
		if len(diff) != 0 {
...
func list_md5(mrafolder string) (filename string, e error) {
	...
	e = md5.collect_all_mra()
	if e != nil {
		return "", e
	}
	md5.sort_by_name()
	_, e = md5.print_report()
	if e != nil {
		return "", e
	}
	filename = "md5.json"
	e = md5.dump_as_json(filename)
	if e != nil {
		return "", e
	}
	return filename, nil
...
func (clc *md5Collector) dump_as_json(filename string) error {
	encoded, e := json.Marshal(clc.all_mra)
	if e != nil {
		return e
	}
...
func compare(newfile, reference string) (diff []string, e error) {
	...
	new, e = read_md5_json(newfile)
	if e != nil {
		return nil, fmt.Errorf("while reading %s\n%w", newfile, e)
	}
	ref, e = read_md5_json(reference)
	if e != nil {
		return nil, fmt.Errorf("while reading %s\n%w", reference, e)
	}
