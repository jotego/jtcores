...
func (w *wav_parser_t) find_signal() error {
	...
	switch len(matches) {
	case 0:
		return fmt.Errorf("Signal %s not found", w.signal_name)
	case 1:
		w.signal = matches[0]
	default:
		return fmt.Errorf("Too many matches for name %s", w.signal_name)
	}
	return nil
}
...
func (w *wav_parser_t) get_16bit_value() int16 {
	if w.signal.MSB == 15 {
		return int16(w.signal.Value)
	}
	if w.signal.MSB < 15 {
		return int16(w.signal.Value << (15 - w.signal.MSB))
	}
	// > 15
