...
func (reg *Register) parse_event_location() (byte_addr, bit int, has_bit bool, e error) {
	...
	if e != nil {
		return 0, 0, false, fmt.Errorf("While parsing address for register %s, found %w", reg.Name, e)
	}
	byte_addr = int(a)
	if matches[2] == "" {
		return byte_addr, 0, false, nil
	}
	if matches[3] != "" {
	...
	if e != nil {
		return 0, 0, false, fmt.Errorf("While parsing bit for register %s, found %w", reg.Name, e)
	}
	bit = int(a)
	if bit < 0 || bit > 7 {
