...
func (this *RegCfg) HasRegionPattern() bool {
	return strings.ContainsAny(this.EffName(), "?*")
}

func (this *RegCfg) MatchRegion(name string) bool {
	eff_name := this.EffName()
	if !this.HasRegionPattern() {
		return eff_name == name
	}
	matched, e := filepath.Match(eff_name, name)
