...
func (cen_cfg *ClockCfg) parse_settings(base_clk string) {
	if e := cen_cfg.parse_freq(); e != nil {
		fmt.Printf("Error: %s for clock %s in mem.yaml\n", e.Error(), base_clk)
		os.Exit(1)
	}
...
func (cen_cfg *ClockCfg) set_name(base_clk string) {
	...
	if mode96 { // clk is 96MHz
		...
		if cen_cfg.ClkName == "clk48" && !mode96 {
			cen_cfg.ClkName = "clk"
		}
	}
}
...
func (cen_cfg *ClockCfg) parse_freq() error {
	s := strings.TrimSpace(cen_cfg.Freq)
	cen_cfg.freq = 0
	if s == "" {
		return nil
	}
	if strings.HasSuffix(s, "Hz") {
		...
		if s == "" {
			return fmt.Errorf("empty frequency value")
		}
	}
	...
	if mul != 1.0 {
		...
		if s == "" {
			return fmt.Errorf("invalid frequency %q", cen_cfg.Freq)
		}
	}
	s = strings.TrimSpace(s)
	v, e := strconv.ParseFloat(s, 64)
	if e != nil {
		return fmt.Errorf("cannot parse frequency %q", cen_cfg.Freq)
	}
	cen_cfg.freq = v * mul
	if cen_cfg.freq < 1.0 {
