...
output  reg     [ 3:0]  mouse_en,   // high if the is a mouse
                        keyb_en,    // high if the device is a keyboard
                        analog_en,  // high if the device has analog sticks
...
wire is_mouse, is_keyb, is_pocket, is_analog;
...
assign is_mouse  = rx_word[31:28]==DOCKED_MOUSE;
assign is_keyb   = rx_word[31:28]==DOCKED_KEYBOARD;
assign is_analog = rx_word[31:28]==DOCKED_ANALOG;
...
always @(posedge clk) begin
    ...
    ST_RX_BUTTON_2: begin
        if(rx_word_done) begin
            ...
            case(cnt)
            0: begin
                cont1_key   <= rx_word[15:0];
                mouse_en[0] <= is_mouse;
                analog_en[0]<= is_analog;
                keyb_en[0]  <= is_keyb;
            end
            1: cont1_joy    <= rx_word;
            ...
            3: begin
                cont2_key   <= rx_word[15:0];
                mouse_en[1] <= is_mouse;
                analog_en[1]<= is_analog;
                keyb_en[1]  <= is_keyb;
            end
            4: cont2_joy    <= rx_word;
            ...
            6: begin
                cont3_key   <= rx_word[15:0];
                mouse_en[2] <= is_mouse;
                analog_en[2]<= is_analog;
                keyb_en[2]  <= is_keyb;
            end
            7: cont3_joy    <= rx_word;
            ...
            9: begin
                cont4_key   <= rx_word[15:0];
                mouse_en[3] <= is_mouse;
                analog_en[3]<= is_analog;
                keyb_en[3]  <= is_keyb;
            end
