...
///	output wire [3:0] DBG_TX
	...
	always @(posedge i_clk) begin
		// NOTE: i_rst (= reset_on_change) is held high for the entire core reset duration.
		// Adding a reset branch here blocks all captures during that period, causing a
		// multi-second initialization delay. FPGA registers default to 0 at power-up:
		// key1/key2=0 (no buttons), att1r/att2r=0, no_gamepad=0.
		// No spurious ATT edge occurs during reset because dualshock_controller holds
		// PSX_ATT1/ATT2 high (idle) while i_RSTn is asserted.
		// if (i_rst) begin
		// 	key1       <= 16'h0000;
		// 	key2       <= 16'h0000;
		// 	joy1       <= 32'h80808080;
		...
		end else if(~att2r & PSX_ATT2) begin //capture when ATT2 becomes idle
			if(no_gamepad) begin //no gamepad detected, default data
				key2 <= 16'h00;
