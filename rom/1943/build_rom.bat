...
:MENU
cls
echo 1943's Arcade Rom Creator
echo.
echo 1 - 1943: The Battle of Midway (Euro) - 1943.zip (Mame)
echo 2 - 1943: The Battle of Midway (US, Rev C) - 1943u.zip (Mame)
...
echo.
For /F "Tokens=*" %%I in ('"!pwd!7za" x -y -otmp %zip1%') Do Set StrAnswer=%%RESUNZ

	if !ERRORLEVEL! EQU 0 ( 
		cd tmp
		echo Creating rom file...
		echo.
		For /F "Tokens=*" %%I in ('copy /b /y %ifiles% "!pwd!%ofile%"') Do Set StrAnswer=%%COPYUNZ
			...
			if !ERRORLEVEL! EQU 0 ( 
				...
				if "%md5valid%" EQU "!md5!" (
					...
					echo ** done **
					echo.
					echo Copy "%ofile%" into root in SD card
