...
export JOTEGO=/jtcores/.jtsupport
...
# accept either jpg or png frame dumps
FRAME_EXT=jpg
ls "$FRAMES_DIR"/frame_*.png >/dev/null 2>&1 && FRAME_EXT=png
if [ -d "$FRAMES_DIR" ] && ls "$FRAMES_DIR"/frame_*.$FRAME_EXT >/dev/null 2>&1; then
    echo
...
PY
    ...
    RAW_IN="$JTROOT/cores/$CORE/ver/game/sound.raw"
    # container may lack raw2wav; convert the raw dump ourselves
    if [ ! -s "$WAV_IN" ] && [ -s "$RAW_IN" ]; then
        ffmpeg -y -f s16le -ar 48000 -ac 2 -i "$RAW_IN" "$WAV_IN" 2>/dev/null || true
    fi
    if file "$WAV_IN" 2>/dev/null | grep -q "WAVE" || [ -s "$WAV_IN" ]; then
