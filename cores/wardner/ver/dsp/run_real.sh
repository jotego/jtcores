...
fail=0; itot=0; ttot=0
for s in $(seq 0 $((SEEDS-1))); do
    # --- instruction level
    "$REF/ref32010" dsp_real.hex 0 toaplantrace "$INSNS" "$s" > ref_it.trace 2>/dev/null
    ...
    if diff -q a.t b.t >/dev/null; then itot=$((itot+c));
    else echo "seed $s: instruction trace diverges"; diff a.t b.t | head -6; fail=1; fi

    # --- transaction level
    "$REF/ref32010" dsp_real.hex 0 toaplanfree "$INSNS" "$s" > ref_tx.tlog 2>/dev/null
