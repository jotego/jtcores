...
{
public:
	enum class replacement_policy : u8
	{
		RANDOM = 0,
		LRU = 1
	};

	static constexpr u32 MAX_BLOCK_COUNT = 256;
	...
	void reset()
	{
		...
		m_policy = replacement_policy::RANDOM;
		m_access_sequence = 0;
		m_average_index = 0;
		m_average_count = 0;
		m_average_total = 0;
	...
	void configure(u32 block_count, u32 tiles_per_block, replacement_policy policy)
	{
		if (m_configured_block_count == block_count && m_configured_tiles_per_block == tiles_per_block && m_policy == policy)
			return;
	...
	void request_tile(u32 tile_code, u32 replacement_index)
	{
		...
		for (u32 index = 0; index < m_configured_block_count; index++)
		{
			...
			{
				m_last_use[index] = m_access_sequence;
				return;
			}
		}

		m_frame_sdram_requests++;
		u32 const block = find_replacement_block(replacement_index);
		m_cached_tile_bases[block] = tile_base;
		m_block_valid[block] = true;
		m_last_use[block] = m_access_sequence;
	}
...
private:
	u32 find_replacement_block(u32 replacement_index) const
	{
		for (u32 index = 0; index < m_configured_block_count; index++)
		{
			if (!m_block_valid[index])
				return index;
		}

		if (m_policy == replacement_policy::RANDOM)
			return replacement_index % m_configured_block_count;

		u32 oldest_index = 0;
		u32 oldest_use = m_last_use[0];
		for (u32 index = 1; index < m_configured_block_count; index++)
		{
			if (m_last_use[index] < oldest_use)
			{
				oldest_use = m_last_use[index];
				oldest_index = index;
			}
		}
	...
	std::array<u32, MAX_BLOCK_COUNT> m_last_use{};
	...
	replacement_policy m_policy = replacement_policy::RANDOM;
	u32 m_access_sequence = 0;
...
public:
	...
	void add_frame(report const &frame)
	{
		...
		m_sdram_requests_total += frame.sdram_requests;
		m_cache_kb_total += frame.cache_kb;
		m_usage_pct_total += frame.usage_pct;

		m_last_report.unique_codes = round_div(m_unique_codes_total, m_frame_count);
		m_last_report.sdram_requests = round_div(m_sdram_requests_total, m_frame_count);
		m_last_report.cache_kb = round_div(m_cache_kb_total, m_frame_count);
		m_last_report.usage_pct = round_div(m_usage_pct_total, m_frame_count);
...
private:
	...
	u64 m_sdram_requests_total = 0;
	u64 m_cache_kb_total = 0;
	u64 m_usage_pct_total = 0;
...
void cps3_state::draw_tilemapsprite_line(u32 *regs, int drawline, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	{
        // tilemap read
		u32 const dat = m_spriteram[mapbase + ((tileline & 63) <<6) + (((x + (scrollx >>4)) & 63) ^ xflip_mask)];
...
u32 cps3_state::screen_update(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	m_tilemap_cache_stats->configure(cache_blocks, cache_tiles, cache_policy);
	...
	int const spritelist_entries = std::min<int>(m_spritelist_dma_sprite_count, SPRITELIST_WORDS / 4);
	for (int entry = 0; entry < spritelist_entries; entry++)
	{
		int const i = entry * 4;
	...
	u32 const sdram_requests = m_tilemap_cache_stats->sdram_request_count();
	u64 const sdram_usage_num = u64(sdram_requests) * u64(m_tilemap_cache_stats->tiles_per_block()) * 256U * FRAME_RATE_X100;
	u32 const sdram_usage_pct = (sdram_usage_num + (FPGA_CLOCK / 2)) / FPGA_CLOCK;
	cps3_report_averager::report const frame_report{
		m_tilemap_cache_stats->unique_tile_count(),
		sdram_requests,
		cache_size_kb,
		sdram_usage_pct
	};
...
bool cps3_state::dump_ss_snapshot(char const *filename)
{
	emu_file file(machine().options().plugin_data_path(), OPEN_FLAG_WRITE | OPEN_FLAG_CREATE | OPEN_FLAG_CREATE_PATHS);
	std::error_condition const filerr = file.open(util::string_format("debug" PATH_SEPARATOR "%s" PATH_SEPARATOR "%s", machine().basename(), filename));
	if (filerr)
	{
		logerror("Unable to create snapshot dump file %s: %s\n", filename, filerr.message());
		return false;
	}
...
{
	u16 const dmast = u16(m_spritelist_dma_sprite_count);
	auto const pack_u32_be = [] (u32 const *src, std::size_t words)
	{
		std::vector<u8> data(words * sizeof(u32));
		for (std::size_t i = 0; i < words; ++i)
		{
			u32 const value = src[i];
			data[i * 4 + 0] = (value >> 24) & 0xff;
	...
	std::array<u8, 0xb0> ppu_regs{};
	auto const put_u32_be = [&ppu_regs] (std::size_t offset, u32 value)
	{
		ppu_regs[offset + 0] = (value >> 24) & 0xff;
		ppu_regs[offset + 1] = (value >> 16) & 0xff;
		ppu_regs[offset + 2] = (value >> 8) & 0xff;
		ppu_regs[offset + 3] = value & 0xff;
	};

	for (std::size_t i = 0; i < 8; ++i)
		put_u32_be(i * 4, m_ppu_gscroll[i]);

	for (std::size_t i = 0; i < 16; ++i)
		put_u32_be(0x20 + i * 4, m_tilemap_regs[i]);

	for (std::size_t i = 0; i < 8; ++i)
		put_u32_be(0x60 + i * 4, m_ppu_crtc_zoom[i]);

	auto const spriteram_be = pack_u32_be(&m_spriteram[0], 0x80000 / 4);
	auto const tilechar_be = pack_u32_be(m_char_ram.get(), 0x800000 / 4);
	auto const scene_be = pack_u32_be(m_spritelist.get(), 0x2000 / 4);
	...
	bool const ok =
			...
			dump_ss_file("ppureg.bin", ppu_regs.data(), ppu_regs.size()) &&
			dump_ss_file("spriteram.bin", spriteram_be.data(), spriteram_be.size()) &&
			dump_ss_file("tilechar.bin", tilechar_be.data(), tilechar_be.size()) &&
			dump_ss_file("dmast.bin", &dmast, sizeof(dmast)) &&
			dump_ss_file("scene.bin", scene_be.data(), scene_be.size()) &&
			dump_ss_file("pal.bin", &m_colourram[0], 0x40000) &&
			dump_ss_snapshot("snapshot.png");
...
void cps3_state::cps3_map(address_map &map)
{
	...
	map(0x05140000, 0x05140003).rw("wd33c93", FUNC(wd33c93_device::indir_r), FUNC(wd33c93_device::indir_w)).umask32(0x00ff00ff);
...
static INPUT_PORTS_START( cps3 )
	...
	PORT_START("CACHE_POLICY")
	PORT_CONFNAME(0x01, 0x00, "Tile Cache Policy")
	PORT_CONFSETTING(0x00, "Random")
	PORT_CONFSETTING(0x01, "LRU")
INPUT_PORTS_END
...
void cps3_state::cps3(machine_config &config)
{
	...
	scsi.set_external_device(1, cdrom);

	auto &wd33c93(WD33C93A(config, "wd33c93", 10'000'000));
