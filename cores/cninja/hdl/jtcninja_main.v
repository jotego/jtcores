...
wire [ 2:0] cn_ipl, cb_ipl, ds_ipl, vp_ipl;
wire [15:0] cn_din, cb_din, ds_din, vp_din;
wire        cn_rom, cb_rom, ds_rom, vp_rom;
wire        cn_ram, cb_ram, ds_ram, vp_ram;
wire        cn_pal, cb_pal, ds_pal, vp_pal;
wire        cn_obj, cb_obj, ds_obj, vp_obj;
wire        cn_pf0, cb_pf0, ds_pf0, vp_pf0;
wire        cn_pf1, cb_pf1, ds_pf1, vp_pf1;
wire        cn_prot,cb_prot,ds_prot,vp_prot;
wire        cn_copy,cb_copy,ds_copy,vp_copy;
wire        cn_sndw,cb_sndw,ds_sndw,vp_sndw;
wire [ 7:0] cn_sndd,cb_sndd,ds_sndd,vp_sndd;
wire        cn_ack, cb_ack, ds_ack, vp_ack;
wire        cb_pri;                       // cbuster TC-4 m_pri
wire [15:0] vp_prio0, vp_prio1;           // vaportra m_priority[0..1]
wire [ 2:0] IPLn = cn_ipl & cb_ipl & ds_ipl & vp_ipl;   // idle = 3'b111
wire        vbl_ack  = cn_ack | cb_ack | ds_ack | vp_ack;
wire        ram_cs   = cn_ram | cb_ram | ds_ram | vp_ram;
wire [15:0] din_mux  = cn_din | cb_din | ds_din | vp_din;

assign rom_cs    = cn_rom | cb_rom | ds_rom | vp_rom;
assign pal_cs    = cn_pal | cb_pal | ds_pal | vp_pal;
assign objram_cs = cn_obj | cb_obj | ds_obj | vp_obj;
assign pf0_cs    = cn_pf0 | cb_pf0 | ds_pf0 | vp_pf0;
assign pf1_cs    = cn_pf1 | cb_pf1 | ds_pf1 | vp_pf1;
assign prot_cs   = cn_prot| cb_prot| ds_prot| vp_prot;
assign obj_copy  = cn_copy| cb_copy| ds_copy| vp_copy;
assign snd_wr    = cn_sndw| cb_sndw| ds_sndw| vp_sndw;
assign snd_dout  = cn_sndd| cb_sndd| ds_sndd| vp_sndd;
...
// vblank can land during the masked init and preempt it.
...
`else
    ...
    wire _unused_nomain = &{ 1'b0, din_mux, IPLn, rom_ok, dip_pause, work_dout,
                             ...
                             dipsw, vdump, cb_pri, vp_prio0, vp_prio1 };
`endif
...
jtcninja_decoder u_cninja(
    ...
    .vbl_irq(vbl_irq), .vbl_ack(cn_ack), .IPLn(cn_ipl),
...
jtcbuster_decoder u_cbuster(
    ...
    .A(A), .cpu_dout(cpu_dout), .RnW(RnW), .UDSn(UDSn), .LDSn(LDSn), .busn(BUSn),
    .vdump(vdump), .LVBL(LVBL), .LHBL(LHBL),
    .vbl_irq(vbl_irq), .vbl_ack(cb_ack), .IPLn(cb_ipl),
...
jtdarkseal_decoder u_darkseal(
    ...
    .A(A), .cpu_dout(cpu_dout), .RnW(RnW), .UDSn(UDSn), .LDSn(LDSn), .busn(BUSn),
    .vdump(vdump), .LVBL(LVBL), .LHBL(LHBL),
    .vbl_irq(vbl_irq), .vbl_ack(ds_ack), .IPLn(ds_ipl),
...
jtvaportra_decoder u_vaportra(
    ...
    .A(A), .cpu_dout(cpu_dout), .RnW(RnW), .UDSn(UDSn), .LDSn(LDSn), .busn(BUSn),
    .vdump(vdump), .LVBL(LVBL), .LHBL(LHBL),
    .vbl_irq(vbl_irq), .vbl_ack(vp_ack), .IPLn(vp_ipl),
