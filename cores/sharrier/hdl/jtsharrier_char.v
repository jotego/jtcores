...
module jtsharrier_char(
    input              rst,
    input              clk,
    input              pxl2_cen,  // pixel clock enable (2x)
    input              pxl_cen,   // pixel clock enable
    input              alt_en,
    // 'V' glyph fix. 1 = correct the rotated glyph, 0 = faithful.
    input              vfix_en,
...
// 'V' GLYPH FIX. Tile 0x56's eight rows are rotated down by one in the ROM,
// so the bottom border row appears at the top. That is authentic: it looks
// the same in MAME and on an original cabinet. The dip ships Off, so the
// core renders the glyph as the PCB does unless the player asks otherwise.
wire [2:0] vrow = (vfix_en && code==9'h056) ? vf[2:0]+3'd1 : vf[2:0];
assign char_addr = { code, vrow };
...
always @(*) begin
    ...
    // Hang-On hardware reads the per-column vertical scroll table at word
    // 0x798. MAME's tilemap_16a path, which TILEMAP_HANGON uses, indexes
    // textram[0xf30/2 + ...], and segaic16.cpp labels F30-F7D as the
    // per-16px-column vertical scroll. jts16 uses 0x780, which suits the
    // S16A titles it targets -- none of those assert column scroll.
    if( col_rd ) begin
        scan_addr = MODEL ? { 5'h1e, hdump[0], hscan_mux[8:4] } :
                            ( 11'h798 + {hscan_mux[8:4],1'b0} + {10'd0,hdump[0]} );
    end
end
