...
always @(posedge clk) begin
    if( rst ) begin
        ...
        if( hs & ~hs_l ) begin
            ...
            1: st <= 2;                     // BRAM latency
            2: st <= 3;
            // MAME clamps the pointer to 0x1FC, not 0x1F8, and then loops
            // while start != first-4. So a pointer above 0x1F8 means an EMPTY
            // list and nothing is drawn. Clamping to LAST instead would draw
            // slot 0x1F8 once - one stale sprite left over from the previous
            // scene, which is exactly what that bug looked like.
            3: begin
