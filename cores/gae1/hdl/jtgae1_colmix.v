...
module jtgae1_colmix (
    ...
    output reg [9:0]  scr_pal_addr,
    input      [15:0] scr_pal_dout,

    input  [12:0] obj_pxl,
    output [ 9:0] obj_pal_addr,
    input  [15:0] obj_pal_dout,
...
wire        opaque0 = gfx_en[1] & (scr0_pxl[3:0] != 4'd0);
wire        opaque1 = gfx_en[2] & (scr1_pxl[3:0] != 4'd0);
wire [ 4:0] rank0   = rank(1'b0, scr0_pxl[3], scr0_pxl[11:10]);
wire [ 4:0] rank1   = rank(1'b1, scr1_pxl[3], scr1_pxl[11:10]);
wire [ 3:0] pri0    = opaque0 ? pri_code(scr0_pxl[3], scr0_pxl[11:10]) : 4'd0;
wire [ 3:0] pri1    = opaque1 ? pri_code(scr1_pxl[3], scr1_pxl[11:10]) : 4'd0;
...
always @(posedge clk) if (pxl_cen) begin
    ...
    if (opaque0 & l0_wins) begin
        scr_pal_addr <= scr0_pxl[9:0];
        win_rank     <= rank0;
        win_opaque   <= 1'b1;
    end else if (opaque1) begin
        scr_pal_addr <= scr1_pxl[9:0];
        win_rank     <= rank1;
        win_opaque   <= 1'b1;
    end else begin
        scr_pal_addr <= 10'd0;
...
wire [3:0] spr_pen = obj_pxl[3:0];
wire [2:0] spr_pri = obj_pxl[12:10];

assign obj_pal_addr = obj_pxl[9:0];
...
wire       bigkarnk_spr_blocked = win_opaque & (((16'd1 << win_prio) & spr_mask) != 16'd0);
...
wire [4:0] red_scr   = scr_pal_dout[4:0];
wire [4:0] green_scr = scr_pal_dout[9:5];
wire [4:0] blue_scr  = scr_pal_dout[14:10];
wire [4:0] red_spr   = obj_pal_dout[4:0];
wire [4:0] green_spr = obj_pal_dout[9:5];
wire [4:0] blue_spr  = obj_pal_dout[14:10];
