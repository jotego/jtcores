#!/bin/bash
# Burst-capture wrapper for Galivan: one MAME run, 15 scene snapshots.
...
cd "$HERE"
count=0
for dump in /tmp/nnjema_gburst_*_dump.bin; do
    [ -f "$dump" ] || continue
    frame=$(echo "$dump" | sed -E 's|.*_gburst_([0-9]+)_dump\.bin|\1|')
    dst="$HERE/burst_${frame}"
    mkdir -p "$dst"
    cp "$dump" "$dst/dump.bin"
    [ -f "/tmp/nnjema_gburst_${frame}_screen.png" ] && \
        cp "/tmp/nnjema_gburst_${frame}_screen.png" "$dst/screen.png"
