/*  This file is part of JTCORES.
    ...
    Reference for protocol semantics:
      - MAME taito/taitosnd.cpp  (mirrored in cores/taitob/doc/)
      - Arcade-TaitoF2_MiSTer rtl/tc0140syt.sv (read-only reference).
        Written fresh here; no verbatim copy of GPLv2 code into this
        GPLv3 file.

    Ported into the taitob core verbatim from
    cores/superman/hdl/jtsuperman_syt.v (Andrea Bogazzi, 5-2026).
    The TC0140SYT silicon is identical between the Taito X System
    (Superman) and the Taito B System (Rastan Saga 2 / Nastar) —
    same chip mask, same protocol, same Z80 memory decoder.
*/
...
`ifdef SIMULATION
                        $display("[%0t] MASTER PORT_W data=%01x  master_idx<=%0d",
                                 $time, main_dout_d, main_dout_d);
`endif
                    end else begin
                        case (master_idx)
                            ...
                            4'd4: begin
                                reset_reg <= (main_dout_d != 4'd0);
`ifdef SIMULATION
                                $display("[%0t] SYT reset_cb: data=%01x → reset_reg=%b",
                                         $time, main_dout_d, (main_dout_d != 4'd0));
`endif
                            end
                            default: ;
                        endcase
`ifdef SIMULATION
                        $display("[%0t] MASTER COMM_W mainmode=%0d data=%01x",
                                 $time, master_idx, main_dout_d);
`endif
                    end
