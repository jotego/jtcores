...
GSH = 2               # output post-gain 2^GSH: R1 signals sit well below
                      # OUT_FS, reclaim headroom at the 16-bit output
...
class BitTrue:
    ...
    def tick_raw(self, x, level):
        ...
        for c in CH:
            self.x1[c] = x[c]
        om = sat(acc['music'] >> (BSH-GSH), 17)
        om = sat((om*self.duck_q) >> 15, 16) if level else sat(om, 16)
        return (om, sat(acc['speech'] >> (BSH-GSH), 16),
                sat(acc['dac'] >> (BSH-GSH), 16))
...
def emit(models, freqs):
    ...
    for ch in CH:
        ...
        Hx = tf_eval(poles, resid, d, f_[1:])*SRC_FS[ch]/OUT_FS*(1 << GSH)
        mag = np.abs(Hx)
