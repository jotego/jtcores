...
protected:
	...
	void capture_scene_raster_until(int screen_y);

private:
	static constexpr int SCENE_RASTER_LINES = 224;

	void scene_frame();
	void dump_scene();
	void write_scene_dump();
	void write_scene_snapshot();
	void append_words_le(std::vector<u8> &dump, u16 const *base, u32 words) const;
	void append_toki_mmr(std::vector<u8> &dump) const;
	void append_scene_raster(std::vector<u8> &dump) const;
	void append_u16le(std::vector<u8> &dump, u16 data) const;
	void store_mmr_word(std::vector<u8> &mmr, u32 offset, u16 data) const;

	int m_dump_at_frame = -1;
	int m_scene_frame = 0;
	int m_scene_raster_next = 0;
	bool m_scene_dumped = false;
	std::string m_dump_path;
	std::array<std::array<u16, 5>, SCENE_RASTER_LINES> m_scene_raster = {};
};
...
void toki_state::control_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	...
	if (vpos <= m_screen->visible_area().max_y)
		capture_scene_raster_until(vpos - 1);
...
{
	m_dump_at_frame = machine().options().dump_at_frame();
	char const *const dump_path = machine().options().dump_path();
	m_dump_path = dump_path ? dump_path : "";

	if (m_dump_at_frame >= 0)
	{
		if (m_dump_path.empty())
			fatalerror("-dump_at_frame requires -dump_path\n");

		machine().add_notifier(MACHINE_NOTIFY_FRAME, machine_notify_delegate(&base_state::scene_frame, this));
	}

	save_item(NAME(m_scene_frame));
	save_item(NAME(m_scene_raster_next));
	save_item(NAME(m_scene_dumped));
}

void base_state::scene_frame()
{
	if (m_dump_at_frame < 0)
		return;
	...
	capture_scene_raster_until(m_screen->visible_area().max_y);

	m_scene_frame++;
	if (m_scene_frame == m_dump_at_frame)
	{
		dump_scene();
		m_scene_dumped = true;
	...
	m_scene_raster_next = 0;
}

void base_state::dump_scene()
{
	write_scene_dump();
	write_scene_snapshot();
	osd_printf_info("Toki scene dumped at frame %d to %s\n", m_scene_frame, m_dump_path.c_str());
}

void base_state::write_scene_dump()
{
	std::vector<u8> dump;
	dump.reserve(0x2800 + 0x80 + SCENE_RASTER_LINES * 10);

	// JTFRAME Toki scene payload, in generated dump2bin.sh order:
	// pal, vram/fix, scr1, scr2, object BRAM, then MMR bytes
	// followed by the raster scroll state used by partial screen updates.
	append_words_le(dump, reinterpret_cast<u16 const *>(m_palette->basemem().base()), 0x400);
	append_words_le(dump, m_videoram.begin(), 0x400);
	append_words_le(dump, m_background_videoram[0].begin(), 0x400);
	append_words_le(dump, m_background_videoram[1].begin(), 0x400);
	append_words_le(dump, m_spriteram->buffer(), m_spriteram->bytes() / 2);
	append_toki_mmr(dump);
	append_scene_raster(dump);

	emu_file file(m_dump_path, OPEN_FLAG_WRITE | OPEN_FLAG_CREATE | OPEN_FLAG_CREATE_PATHS);
	std::error_condition const filerr = file.open("dump.bin");
	if (filerr)
	{
		osd_printf_error("Error opening Toki scene dump.bin (%s:%d %s)\n", filerr.category().name(), filerr.value(), filerr.message());
		return;
	}

	if (file.write(dump.data(), dump.size()) != dump.size())
		osd_printf_error("Error writing Toki scene dump.bin\n");
}

void base_state::write_scene_snapshot()
{
	emu_file file(m_dump_path, OPEN_FLAG_WRITE | OPEN_FLAG_CREATE | OPEN_FLAG_CREATE_PATHS);
	std::error_condition const filerr = file.open("snapshot.png");
	if (filerr)
	{
		osd_printf_error("Error opening Toki scene snapshot.png (%s:%d %s)\n", filerr.category().name(), filerr.value(), filerr.message());
		return;
	}

	machine().video().save_snapshot(m_screen.target(), file);
}

void base_state::append_words_le(std::vector<u8> &dump, u16 const *base, u32 words) const
{
	for (u32 offset = 0; offset < words; offset++)
		append_u16le(dump, base[offset]);
}

void base_state::append_toki_mmr(std::vector<u8> &dump) const
{
	std::vector<u8> mmr(0x80, 0);

	store_mmr_word(mmr, 0x05, m_scrollram[0x05]);
	store_mmr_word(mmr, 0x06, m_scrollram[0x06]);
	store_mmr_word(mmr, 0x0d, m_scrollram[0x0d]);
	store_mmr_word(mmr, 0x0e, m_scrollram[0x0e]);
	store_mmr_word(mmr, 0x15, m_scrollram[0x15]);
	store_mmr_word(mmr, 0x16, m_scrollram[0x16]);
	store_mmr_word(mmr, 0x1d, m_scrollram[0x1d]);
	store_mmr_word(mmr, 0x1e, m_scrollram[0x1e]);
	store_mmr_word(mmr, 0x28, m_scrollram[0x28]);

	dump.insert(dump.end(), mmr.begin(), mmr.end());
}

void base_state::append_scene_raster(std::vector<u8> &dump) const
{
	for (auto const &line : m_scene_raster)
		for (u16 data : line)
			append_u16le(dump, data);
}

void base_state::append_u16le(std::vector<u8> &dump, u16 data) const
{
	dump.push_back(data & 0xff);
	dump.push_back(data >> 8);
}

void base_state::store_mmr_word(std::vector<u8> &mmr, u32 offset, u16 data) const
{
	mmr[offset * 2] = data & 0xff;
	mmr[offset * 2 + 1] = data >> 8;
}

void base_state::capture_scene_raster_until(int screen_y)
{
	if (m_dump_at_frame < 0 || m_scene_dumped)
		return;

	rectangle const &visible = m_screen->visible_area();
	int const end_line = std::min(screen_y, visible.max_y) - visible.min_y;

	if (end_line < m_scene_raster_next)
		return;

	int const scr1_scroll_x = ((m_scrollram[0x06] & 0x7f) << 1) | ((m_scrollram[0x06] & 0x80) >> 7) | ((m_scrollram[0x05] & 0x10) << 4);
	int const scr1_scroll_y = ((m_scrollram[0x0d] & 0x10) << 4) + ((m_scrollram[0x0e] & 0x7f) << 1) + ((m_scrollram[0x0e] & 0x80) >> 7);
	int const scr2_scroll_x = ((m_scrollram[0x16] & 0x7f) << 1) | ((m_scrollram[0x16] & 0x80) >> 7) | ((m_scrollram[0x15] & 0x10) << 4);
	int const scr2_scroll_y = ((m_scrollram[0x1d] & 0x10) << 4) + ((m_scrollram[0x1e] & 0x7f) << 1) + ((m_scrollram[0x1e] & 0x80) >> 7);
	u16 const bg_order = (m_scrollram[0x28] & 0x100) ? 1 : 0;
	int const clamped_end = std::min(end_line, SCENE_RASTER_LINES - 1);

	for (int line = m_scene_raster_next; line <= clamped_end; line++)
		m_scene_raster[line] = {
			u16(scr1_scroll_x),
			u16(scr1_scroll_y),
			u16(scr2_scroll_x),
	...
	m_scene_raster_next = clamped_end + 1;
}
