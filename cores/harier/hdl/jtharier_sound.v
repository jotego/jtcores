...
module jtsharrier_sound(
    input                rst,
    input                clk,

    input                rstn,      // PPI0 port B bit 5, held low by the 68000
...
// The 68000 holds the whole sound board in reset through PPI0 port B bit 5
// (CPU sheet 2/6), so the board reset is the system reset OR that line low.
wire snd_rst = rst | ~rstn;
...
// WD=16 defeats the per-voice clip. jtoutrun_pcm applies clipDAC() to each voice
// before accumulating, but WD is the FINAL DAC width and that DAC converts the
// sum. The product (rom_data-0x80) * vol spans +/-16256 and needs 15 bits, so the
// default 12 saturates eight times too early; this driver writes volumes up to
// 0x3F, so everything above a quarter amplitude distorts. segapcm.cpp accumulates
// unclipped.
