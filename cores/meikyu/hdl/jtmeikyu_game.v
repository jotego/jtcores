...
always @(posedge clk) begin
    case(ioctl_addr[1:0])
        0: dump_mux <= scr1pos[7:0];
        1: dump_mux <= 8'd0;
        2: dump_mux <= { 7'd0, scr1pos[8] };
        3: dump_mux <= { 3'd0, flip, 4'd0 };
    endcase
    case(debug_bus[1:0])
        0: debug_mux <= scr1pos[7:0];
        1: debug_mux <= 8'd0;
        2: debug_mux <= { 7'd0, scr1pos[8] };
        3: debug_mux <= { 3'd0, flip, 4'd0 };
    endcase
end

/* verilator tracing_off */
jtmeikyu_main u_main(
    .rst         ( rst        ),
    .clk         ( clk        ),
    .cpu_cen     ( cpu_cen    ),
    // Video
