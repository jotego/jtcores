...
dd if=/dev/zero of=obj.bin  count=16 conv=notrunc oflag=append 2> /dev/null; SKIP=$((SKIP +16)) # 8kB blank
# MMR
dd if=$TMP of=pal_mmr.bin bs=8 count=2 skip=$((SKIP*512/8))   2> /dev/null
dd if=$TMP of=scr_mmr.bin bs=8 count=1 skip=$((SKIP*512/8+2)) 2> /dev/null
dd if=$TMP of=obj_mmr.bin bs=8 count=1 skip=$((SKIP*512/8+3)) 2> /dev/null
dd if=$TMP of=other.bin   bs=1 count=1 skip=$((SKIP*512+4*8)) 2> /dev/null
